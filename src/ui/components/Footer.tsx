...
export function Footer(p: FooterProps) {
  ...
  return (
    <footer class="footer">
      ...
      {p.capture && p.capture.pages > 0 ? <p class="muted small">Read {plural(p.capture.pages, 'page')}{p.capture.viewerHandle ? ` as @${p.capture.viewerHandle}` : ''}.</p> : null}
      {p.message ? <p class="muted small" role="status">{p.message}</p> : null}
